...
def get_ligne(matrice, ligne):
    """renvoie la ligne sous forme de liste dans la matrice

    Args:
        matrice (set): une matrice
        ligne (int): la ligne souhaité

    Returns:
        list: les valeurs de la ligne
    """    
    listligne = []
    start = ligne*get_nb_colonnes(matrice)-get_nb_colonnes(matrice)
    for i in range(start,get_nb_colonnes(matrice)+start):
        listligne.append(matrice[2][i])
    return listligne

def get_colonne(matrice,colonne):
    """renvoie la colonne sous forme de liste dans la matrice

    Args:
        matrice (set): une matrice
        colonne (int): la colonne souhaité

    Returns:
        list: les valeurs de la colonne
    """   
    listcolonne = []
    for i in range(colonne-1,len(matrice[2]),get_nb_colonnes(matrice)):
        listcolonne.append(matrice[2][i])
    return listcolonne

# Pour les autres fonctions on je n'ai pas vu les math 
# qui vont avec donc les termes ne sont pas connus 